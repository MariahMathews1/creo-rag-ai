...
const postInputStatus = (revision?: MachineProfileRevision) => {
  if (!revision) return "Needs Information";
  const values = [revision.machine_type, revision.controller_name || revision.controller_model, revision.axis_count,
    revision.x_min, revision.x_max, revision.z_min, revision.z_max, revision.max_spindle_rpm,
    revision.max_feed_rate, revision.supported_work_offsets_json?.length ? revision.supported_work_offsets_json : null];
  const confirmed = values.filter((value) => value !== null && value !== undefined && value !== "").length;
  return `${confirmed} confirmed · ${values.length - confirmed} missing`;
};
...
export function GPostGeneratorPage() {
  ...
  if (createOpen) return <section className="page gpost-home-page v1-post-create">
    <PageHeader eyebrow={`Create Post · Step ${step === "machine" ? 1 : step === "setup" ? 2 : 3} of 3`} title={step === "machine" ? "Select Machine" : step === "setup" ? "Review Post Inputs" : "Create Post"} description={step === "setup" ? "Confirm the machine context and review what engineering information is currently available before creating the Post Record." : "Create a machine-specific Post Record."} action={<button className="button secondary" onClick={() => setCreateOpen(false)}>Cancel</button>} />
    {error && <p className="form-error" role="alert">{error}</p>}
    <div className="gpost-create-progress" aria-label="Create progress"><span className={step === "machine" ? "active" : "complete"}>1 <b>Machine</b></span><span className={step === "setup" ? "active" : step === "create" ? "complete" : ""}>2 <b>Review Inputs</b></span><span className={step === "create" ? "active" : ""}>3 <b>Create Post</b></span></div>
    ...
    {step === "setup" && selectedMachine && <section className="panel post-setup-summary"><h2>Review Post Inputs</h2><section><h3>Machine</h3><dl><div><dt>Machine</dt><dd>{selectedMachine.name}</dd></div><div><dt>Machine Type</dt><dd>{selectedMachine.machine_type.replaceAll("_", " ")}</dd></div><div><dt>Controller</dt><dd>{selectedMachine.controller_manufacturer || selectedMachine.controller_name} {selectedMachine.controller_model || ""}</dd></div></dl></section><section><h3>Available Engineering Information</h3><dl><div><dt>Machine Information</dt><dd>{postInputStatus(selectedRevision)}</dd></div><div><dt>Documents</dt><dd>{selectedContext?.documents.length ?? 0} available</dd></div><div><dt>Site Standards</dt><dd>{siteStandardCount} available</dd></div></dl></section><section><h3>OFG Starting Reference</h3><dl><div><dt>Suggested Starting Reference</dt><dd>{selectedStartingPoint?.label}<small>Suggested development reference based on machine/controller family. Confirm the actual starting option/template in the installed OFG environment.</small></dd></div></dl></section><p className="field-note">Final implementation and verification occur in the local G-POST environment.</p><footer><button className="button secondary" onClick={() => setStep("machine")}>Back</button><button className="button primary" disabled={!selectedRevision} onClick={() => setStep("create")}>Continue</button></footer></section>}
    {step === "create" && selectedMachine && <section className="gpost-draft-confirm panel"><div><h2>Post identity</h2><label>Post Name<input autoFocus value={postName} onChange={(event) => setPostName(event.target.value)} /></label></div><dl>
      <div><dt>Machine</dt><dd>{selectedMachine.name}</dd></div><div><dt>Controller</dt><dd>{selectedMachine.controller_model || selectedMachine.controller_name}</dd></div><div><dt>OFG Starting Reference</dt><dd>{selectedStartingPoint?.label}</dd></div><div><dt>Documents</dt><dd>{selectedContext?.documents.length ?? 0} selected</dd></div>
  ...
  return <section className="page gpost-home-page v1-post-list">
    ...
    <section className="gpost-drafts-section"><header><div><h2>Posts</h2><p>Current Post Records and their next engineering action.</p></div><span>{rows.length} shown</span></header>
      {!rows.length ? <div className="gpost-empty-state"><h2>No Post Records yet.</h2><p>Create a machine-specific Post Record to begin configuring G-POST.</p></div> : <div className="gpost-draft-table-wrap"><table className="gpost-draft-table"><thead><tr><th>Post</th><th>Machine</th><th>Controller</th><th>Status</th><th>Next Action</th><th>Updated</th><th>Action</th></tr></thead><tbody>{rows.map((post) => {
