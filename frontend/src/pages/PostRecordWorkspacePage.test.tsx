...
const fact = { id: 10, post_record_id: 4, category: "Spindle", fact_key: "max_spindle_rpm", name: "Maximum Spindle RPM", value_json: 2000, unit: "RPM", status: "confirmed", post_review_status: "reviewed_for_post", source_document_id: 3, source_label: "KLS Machine Specification", source_location: "p. 4", reviewer: "Mariah", reviewed_at: "2026-08-19", review_note: null, created_at: "2026-08-18", updated_at: "2026-08-19", used_by: [{ type: "ofg_setting", id: 20, label: "Maximum Spindle Speed" }] };
const proposedFact = { ...fact, id: 11, fact_key: "g74_behavior", name: "G74 Behavior", value_json: "Requires review", unit: null, status: "needs_review", post_review_status: "available_from_machine", reviewed_at: null, used_by: [] };
...
test("Post Record navigation is workflow-oriented and moves specialist tools out", async () => {
  ...
  for (const label of ["Overview", "Machine Information", "OFG Configuration", "Custom Logic", "Review & Export", "Versions & Sources"]) expect(screen.getByRole("link", { name: label })).toBeInTheDocument();
  ...
  expect(screen.queryByText("Technical Details", { selector: "summary" })).not.toBeInTheDocument();
});
test("overview reports engineering counts and one contextual action", async () => {
  ...
  expect(screen.getByText("18 reviewed · 2 remaining")).toBeInTheDocument(); expect(screen.getByText("32 reviewed · 13 remaining")).toBeInTheDocument();
  expect(screen.getAllByRole("link", { name: "Continue Machine Information Review" }).length).toBeGreaterThan(0);
});
...
test("OFG setting detail qualifies reference-backed locations without claiming site verification", async () => {
  ...
  expect(screen.getByRole("link", { name: "View Machine Information" })).toBeInTheDocument(); expect(screen.getByText("Verified From OFG Reference")).toBeInTheDocument();
  expect(screen.getByText("Site Standards")).toBeInTheDocument();
  expect(screen.getByLabelText("Engineer Notes")).toBeInTheDocument();
  const status = screen.getByLabelText("Status");
  expect(Array.from(status.querySelectorAll("option")).map((option) => option.textContent)).toEqual(["Ready to Review", "Ready to Review", "Ready to Review", "Needs Information", "Reviewed", "Not Applicable", "Custom Logic Needed"]);
});
test("OFG custom logic callout explains the reason without claiming a certain implementation", async () => {
  ...
  renderAt("/gpost/4/ofg-configuration/20"); expect(await screen.findByText("G74 Grooving Output")).toBeInTheDocument();
  expect(screen.getByRole("link", { name: "Open related Custom Logic" })).toHaveAttribute("href", "/gpost/4/custom-logic");
  expect(screen.getByText(/Potential FIL\/CIMFIL implementation only until verified/)).toBeInTheDocument();
});
test("OFG checklist uses machine relevance, domain filters, reference paths, and advanced disclosure", async () => {
  ...
  expect(await screen.findByText(/This checklist represents Option File Generator settings believed relevant/)).toBeInTheDocument();
  for (const label of ["Search", "Category", "Status", "Relevance", "Custom Logic"]) expect(screen.getByLabelText(label)).toBeInTheDocument();
  expect(screen.getByText("Spindle", { selector: "span" })).toBeInTheDocument();
  const settingRow = screen.getByText("Maximum Spindle Speed").closest("article")!;
  for (const label of ["Current Value", "Source", "Status"]) expect(within(settingRow).getByText(label)).toBeInTheDocument();
  expect(within(settingRow).getByText("Required for this Post")).toBeInTheDocument();
  ...
  const advancedRow = screen.getByText("MULTAX Motion").closest("article")!;
...
test("structured OFG address rows preserve formats, order, source, and unknown code state", async () => {
  ...
  const structured = screen.getByRole("group", { name: "Structured Values" });
  for (const label of ["Address", "Description", "Output Order", "Before Alias", "After Alias", "Metric Format", "Inch Format", "Status", "Source"]) expect(within(structured).getByLabelText(label)).toBeInTheDocument();
  expect(screen.getByLabelText("Code Status")).toHaveValue("unknown");
});
test("Machine Information separates available and post-reviewed values with review actions and traceability", async () => {
  const user = userEvent.setup(); vi.mocked(api.updateMachineKnowledge).mockResolvedValue({ ...proposedFact, status: "confirmed" } as never);
  renderAt("/gpost/4/machine-knowledge"); expect(await screen.findByRole("heading", { name: "Available from Machine" })).toBeInTheDocument();
  expect(screen.getByRole("heading", { name: "Reviewed for this Post" })).toBeInTheDocument();
  expect(screen.getByText("G74 Behavior")).toBeInTheDocument(); expect(screen.getByText("Not mapped")).toBeInTheDocument();
  expect(screen.queryByRole("button", { name: "Confirm" })).not.toBeInTheDocument();
  await user.click(within(screen.getByText("G74 Behavior").closest("tr")!).getByRole("button", { name: "Review →" }));
  ...
  await user.click(screen.getByRole("button", { name: "Mark Reviewed for Post" }));
...
test("proposed fact drawer supports edit and confirm", async () => {
  ...
  renderAt("/gpost/4/machine-knowledge"); await screen.findByText("G74 Behavior"); await user.click(within(screen.getByText("G74 Behavior").closest("tr")!).getByRole("button", { name: "Review →" }));
  await user.click(screen.getByRole("button", { name: "Edit Value" })); const input = screen.getByLabelText("Edit value"); await user.clear(input); await user.type(input, "Reviewed G74"); await user.click(screen.getByRole("button", { name: "Save & Review" }));
...
test("proposed fact drawer supports reject", async () => {
  ...
  renderAt("/gpost/4/machine-knowledge"); await screen.findByText("G74 Behavior"); await user.click(within(screen.getByText("G74 Behavior").closest("tr")!).getByRole("button", { name: "Review →" })); await user.click(screen.getByRole("button", { name: "Not Applicable" }));
...
test("export explains package contents and the native-post boundary", async () => {
  ...
  for (const item of ["Machine Summary", "Reviewed Machine Information", "OFG Configuration Checklist", "Applied Site Standards", "Custom Logic Summary", "Engineer Questions", "Review Summary", "External Verification Records", "Sources", "Version Metadata"]) expect(within(packagePanel).getByText(item)).toBeInTheDocument();
...
test("workflow concepts expose concise demo tooltips", async () => {
  ...
  expect(screen.getByTitle("Reviewed machine/controller information available to this Post Record.")).toBeInTheDocument();
...
test("Review has one completion summary, outstanding queue, and compact validation stages", async () => {
  ...
  for (const heading of ["Completion", "Outstanding Items", "External Verification"]) expect(screen.getByRole("heading", { name: heading })).toBeInTheDocument();
...
test("Add external verification uses the reduced manual record form", async () => {
...
test("fact review remains manual and works with AI disabled", async () => {
  ...
  renderAt("/gpost/4/machine-knowledge/10"); await user.click(await screen.findByRole("button", { name: "Mark Reviewed for Post" }));
