          ...
          {profiles.map((profile) => (
            <article className="profile-card" key={profile.id}>
              <div className="profile-card-head">
                ...
                <div className="card-actions"><Link to={`/machines/${profile.id}/profile-extraction/new`}>Extract from documents</Link><Link to={`/machines/${profile.id}/revisions`}>Revisions</Link><button onClick={() => { setEditing(profile); setShowForm(true); }}>Edit</button><button className="danger-link" onClick={() => void remove(profile)}>Delete</button></div>
              </div>
