...
import type {
  ...
  GPostHistoricalTranslationEvidence, GPostPreflight,
  ToolpathResponse,
} from "../types";
import {
...
function mappingDisplayName(mapping: GPostMapping) {
  ...
  const names: Record<string, string> = { LOADTL: "Tool Selection", SPINDL: suffix.toLowerCase().includes("ccw") ? "Spindle — Counter-clockwise Start" : suffix.toLowerCase().includes("off") ? "Spindle — Stop" : "Spindle — Clockwise Start", COOLNT: "Coolant Control", FEDRAT: "Feed Rate", RAPID: "Rapid Motion", GOTO: "Programmed Motion", FINI: "Program End", GOHOME: "Reference Return" };
  return names[mapping.cl_command] ?? (suffix ? `${mapping.cl_command} / ${suffix}` : mapping.cl_command);
}
...
export function GPostWorkspacePage() {
  ...
  const routeSuffix = location.pathname.replace(new RegExp(`^/gpost/${id}/?`), "");
  const routeTab: Partial<Record<string, GPostTab>> = { toolpath: "toolpath" as GPostTab, results: "results" as GPostTab, "advanced/readiness": "overview", evidence: "sources", "advanced/templates": "configuration", "advanced/mappings": "mappings", "advanced/validation": "validation", "advanced/versions": "versions" };
  const legacyTab = WORKSPACE_TABS.includes(params.get("tab") as GPostTab) ? params.get("tab") as GPostTab : null;
  const activeTab = (routeTab[routeSuffix] ?? legacyTab ?? "test") as GPostTab;
  const primaryView = routeSuffix === "toolpath" ? "toolpath" : routeSuffix === "results" ? "results" : routeSuffix === "evidence" ? "evidence" : routeSuffix.startsWith("advanced/") ? "advanced" : legacyTab === "sources" ? "evidence" : legacyTab === "test" && params.get("result") === "toolpath" ? "toolpath" : legacyTab === "test" && params.get("result") === "validation" ? "results" : legacyTab && legacyTab !== "test" ? "advanced" : "generate";
  ...
  const [preflight, setPreflight] = useState<GPostPreflight | null>(null);
  ...
  function goTo(view: "generate" | "toolpath" | "results" | "evidence" | "templates" | "mappings" | "validation" | "versions" | "readiness", query: Record<string, string | null> = {}) {
    const paths = { generate: "", toolpath: "toolpath", results: "results", evidence: "evidence", templates: "advanced/templates", mappings: "advanced/mappings", validation: "advanced/validation", versions: "advanced/versions", readiness: "advanced/readiness" };
    const next = new URLSearchParams(); Object.entries(query).forEach(([key, value]) => { if (value) next.set(key, value); });
    navigate(`/gpost/${id}/${paths[view]}${next.size ? `?${next}` : ""}`.replace(/\/$/, ""));
  }
  ...
  useEffect(() => { if (preview && primaryView === "toolpath" && typeof api.getGPostPreviewToolpath === "function") void api.getGPostPreviewToolpath(preview.id).then(setPreviewToolpath).catch((cause) => setError(cause.message)); }, [preview?.id, primaryView]);
  useEffect(() => {
    if (!clSource.trim() || typeof api.preflightGPost !== "function") { setPreflight(null); return; }
    const timer = window.setTimeout(() => { void api.preflightGPost(id, clSource).then(setPreflight).catch((cause) => setError(cause.message)); }, 250);
  ...
  const setupBlockers = preflight ? preflight.blocking_issues.length : [!familyCompatible, !machineTypeMatches, !revisionApproved, !templates.safe_start].filter(Boolean).length;
  ...
  return <section className="page gpost-workspace-page">
    ...
    <header className="gpost-workspace-header"><div><Link to="/gpost" className="gpost-back-link">← G-POST Generator</Link><h1 className="gpost-draft-title">{draft.name}</h1><p>{machine.name} · {machine.controller_manufacturer || machine.controller_name} {machine.controller_model || ""}</p></div><div className="gpost-header-actions"><GPostStatusBadge status={draft.status} large /><button disabled={busy} onClick={() => void saveConfiguration()}>Save</button><button onClick={() => void createVersion()}>New Version</button><details><summary>Export</summary><a href={api.gpostExportUrl(draft.id, "json")}>JSON</a><a href={api.gpostExportUrl(draft.id, "markdown")}>Markdown</a></details><ActionMenu label="More" items={[{ label: "Archive Draft", danger: true, onSelect: () => void api.archiveGPostDraft(draft.id).then(setDraft) }]} /></div></header>
    ...
    <nav className="gpost-workflow-nav v1-gpost-nav" aria-label="G-POST workflow"><Link aria-current={primaryView === "generate" ? "page" : undefined} className={primaryView === "generate" ? "active" : ""} to={`/gpost/${id}`}>Generate</Link><Link aria-current={primaryView === "toolpath" ? "page" : undefined} className={primaryView === "toolpath" ? "active" : ""} to={`/gpost/${id}/toolpath`}>Toolpath</Link><Link aria-current={primaryView === "results" ? "page" : undefined} className={primaryView === "results" ? "active" : ""} to={`/gpost/${id}/results`}>Results</Link><Link aria-current={primaryView === "evidence" ? "page" : undefined} className={primaryView === "evidence" ? "active" : ""} to={`/gpost/${id}/evidence`}>Evidence</Link><ActionMenu label="Advanced Post Configuration" active={primaryView === "advanced"} items={[{ label: "Templates", to: `/gpost/${id}/advanced/templates`, active: activeTab === "configuration" }, { label: "Mappings", to: `/gpost/${id}/advanced/mappings${selectedMapping ? `?mapping=${selectedMapping.mapping_key}` : ""}`, active: activeTab === "mappings" }, { label: "Validation", to: `/gpost/${id}/advanced/validation`, active: activeTab === "validation" }, { label: "Versions", to: `/gpost/${id}/advanced/versions`, active: activeTab === "versions" }, { label: "Technical Readiness", to: `/gpost/${id}/advanced/readiness`, active: activeTab === "overview" }]} /></nav>
    {activeTab === "test" && <section className="panel gpost-v1-generate-context"><header><div><h2>Generate R&amp;D Draft</h2><p>Provide Creo CL/NCL, generate a draft, then inspect checks and toolpath.</p></div><GPostStatusBadge status={setupBlockers ? "blocked" : metrics.blocking ? "pending" : "accepted"} /></header><dl><div><dt>Machine</dt><dd>{machine.name}</dd></div><div><dt>Controller</dt><dd>{revision?.controller_model || revision?.controller_name}</dd></div><div><dt>Post Context</dt><dd>{templateFamilyLabel} post</dd></div><div><dt>Historical Examples</dt><dd>{historicalEvidence?.verified_example_count ?? 0} verified for selected behavior</dd></div><div><dt>Documents</dt><dd>{documents.length} available</dd></div><div><dt>Translation Method</dt><dd>Deterministic / Mock</dd></div></dl>{setupBlockers > 0 && <p className="variant-warning">Machine information or post-template selection must be corrected before generation.</p>}</section>}
    {activeTab === "test" && preflight && <section className={`panel gpost-current-preflight ${preflight.generation_allowed ? "ready" : "blocked"}`}><header><div><span className="eyebrow">Current CL Preflight</span><h2>{preflight.generation_allowed ? preflight.unreviewed_behavior_keys.length ? "Ready for R&D Preview — Review Recommended" : "Ready for R&D Preview" : `Blocked — ${String(preflight.blocking_issues[0]?.title ?? "Needs Attention")}`}</h2></div><GPostStatusBadge status={preflight.generation_allowed ? "under_review" : "blocked"} /></header><dl><div><dt>Machine</dt><dd>{preflight.machine_ready ? "Ready" : "Needs attention"}</dd></div><div><dt>Post Context</dt><dd>{preflight.post_context_ready ? "Ready" : "Needs attention"}</dd></div><div><dt>CL Input</dt><dd>{preflight.cl_record_count} records</dd></div><div><dt>Required Behaviors</dt><dd>{preflight.required_behavior_keys.length}</dd></div><div><dt>Supported</dt><dd>{preflight.supported_behavior_keys.length} / {preflight.required_behavior_keys.length}</dd></div><div><dt>Reviewed</dt><dd>{preflight.reviewed_behavior_keys.length} / {preflight.required_behavior_keys.length}</dd></div><div><dt>Blocking Issues</dt><dd>{preflight.blocking_issues.length}</dd></div><div><dt>Warnings</dt><dd>{preflight.warnings.length}</dd></div></dl>{preflight.unreviewed_behavior_keys.length > 0 && preflight.generation_allowed && <aside className="preflight-warning"><strong>⚠ {preflight.unreviewed_behavior_keys.length} supported behavior(s) have not been manually reviewed.</strong><button onClick={() => updateParams({ tab: "mappings", queue: "needs-review", mapping: preflight.unreviewed_behavior_keys[0] })}>Review Behavior</button></aside>}{preflight.blocking_issues.map((issue, index) => <aside className="preflight-blocker" key={index}><strong>{String(issue.title ?? "Generation Blocked")}</strong><p>{String(issue.message ?? "A required issue must be fixed before generation.")}</p><div>{issue.action === "configure_mapping" && <><button onClick={() => updateParams({ tab: "test" })}>Review CL</button><button className="button primary" onClick={() => updateParams({ tab: "mappings", queue: "blocking", mapping: String((issue.behavior_keys as string[] | undefined)?.[0] ?? "") })}>Configure Mapping</button></>}{issue.action === "change_template" && <button className="button primary" onClick={() => navigate("/gpost")}>Choose Compatible Template</button>}{issue.action === "open_configuration" && <button className="button primary" onClick={() => updateParams({ tab: "overview" })}>Open Configuration</button>}</div></aside>)}</section>}
    {activeTab === "test" && preview && <section className="panel gpost-result-header"><header><div><span className="eyebrow">Generated R&amp;D Draft</span><h2>{preview.status === "blocked" ? `Blocked — ${String(preview.summary_json.blocking_cause ?? "Needs Attention")}` : "R&D Preview Generated"}</h2><p>Programmer review and controlled simulation are still required.</p></div><GPostStatusBadge status={preview.status === "blocked" ? "blocked" : "validated_for_rnd"} large /></header><dl><div><dt>Parser</dt><dd>{preview.parser_diagnostics_json.length ? "Needs review" : "Passed"}</dd></div><div><dt>Command Checks</dt><dd>{preview.unsupported_commands_json.length || preview.missing_mappings_json.length ? "Needs review" : "Passed"}</dd></div><div><dt>Machine Findings</dt><dd>{String(preview.summary_json.deterministic_finding_count ?? 0)}</dd></div><div><dt>Historical Evidence</dt><dd>{historicalEvidence?.verified_example_count ?? 0} examples available</dd></div></dl><div className="gpost-result-actions"><button onClick={() => updateParams({ result: "toolpath" })}>View Toolpath</button><button onClick={() => updateParams({ result: "validation" })}>View Findings</button><button onClick={() => updateParams({ tab: "sources" })}>View Evidence</button><button onClick={() => void saveConfiguration()}>Save Draft</button></div></section>}
    ...
    {primaryView === "toolpath" && <section className="panel gpost-preview-toolpath"><header><h2>Visual Toolpath Review</h2><p>Input CL and generated G-code programmed-motion overlay.</p></header>{!preview ? <div className="gpost-compact-empty"><h2>No generated toolpath yet</h2><p>Generate an R&amp;D draft first to visualize its CL and generated G-code motion.</p><button className="button primary" onClick={() => goTo("generate")}>Go to Generate</button></div> : previewToolpath ? <ToolpathViewer data={previewToolpath} /> : <p>Loading programmed motion…</p>}</section>}
    {primaryView === "results" && !preview && <section className="panel gpost-compact-empty"><h2>No R&amp;D result yet</h2><p>No R&amp;D draft has been generated yet.</p><button className="button primary" onClick={() => goTo("generate")}>Go to Generate</button></section>}
    {primaryView === "results" && preview && <section className="panel gpost-results-workspace"><header><div><span className="eyebrow">Generated R&amp;D Draft</span><h2>{preview.status === "blocked" ? "Generation Blocked" : "Generation Results"}</h2></div><GPostStatusBadge status={preview.status === "blocked" ? "blocked" : "validated_for_rnd"} large /></header><dl><div><dt>Parser</dt><dd>{preview.parser_diagnostics_json.length ? "Findings" : "Passed"}</dd></div><div><dt>Command Checks</dt><dd>{preview.unsupported_commands_json.length || preview.missing_mappings_json.length ? "Findings" : "Passed"}</dd></div><div><dt>Machine Findings</dt><dd>{String(preview.summary_json.deterministic_finding_count ?? 0)} total</dd></div><div><dt>Historical Evidence</dt><dd>{historicalEvidence?.verified_example_count ?? 0} examples used</dd></div><div><dt>Translation Behaviors</dt><dd>{preflight ? `${preflight.supported_behavior_keys.length} supported · ${preflight.unreviewed_behavior_keys.length} unreviewed · ${preflight.unsupported_required_behaviors.length} unsupported` : "Not available"}</dd></div></dl><div className="gpost-result-actions"><button onClick={() => goTo("generate")}>View Generated G-code</button><button onClick={() => goTo("validation")}>View Findings</button><button onClick={() => goTo("toolpath")}>View Toolpath</button><button onClick={() => goTo("evidence")}>View Evidence</button></div></section>}
    ...
    {activeTab === "mappings" && selectedMapping && <section className="panel deprecated-ai-experiment"><h2>Legacy translation-AI experiment disabled</h2><p>CL/NCL transmission to AI is prohibited. This legacy preview remains local and cannot invoke a provider.</p></section>}
