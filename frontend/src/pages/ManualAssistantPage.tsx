...
export function ManualAssistantPage() {
  ...
  return <section className="page">
    <PageHeader eyebrow="Machine-specific reference" title="Machine Assistant" description="Ask questions using documents associated with the selected machine." action={<Link className="button secondary" to={`/documents?machine=${machineId}`}>Manage documents</Link>} />
    ...
    <div className="assistant-layout">
      ...
      <main className="reference-workspace">
        <form className="question-form" onSubmit={submit}><label>Question<textarea aria-label="Technical question" rows={3} value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="What does the manual say about maximum spindle RPM?" /></label><div className="question-options"><label>Category<select value={category} onChange={(event) => setCategory(event.target.value)}>{["general","command_meaning","cycle_support","machine_limit","setup_requirement","tool_change","work_offset","spindle","feed","coolant","alarm","post_processor"].map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}</option>)}</select></label><details><summary>Technical Retrieval Details</summary><fieldset><legend>Limit sources (optional)</legend>{(["controller_manual","programming_manual","company_standard","post_processor_document"] as DocumentType[]).map((item) => <label key={item}><input type="checkbox" checked={types.includes(item)} onChange={() => toggleType(item)} />{item.replaceAll("_", " ")}</label>)}</fieldset></details><button aria-label="Retrieve grounded answer" className="button primary large" disabled={busy || !question.trim() || !machineId}>{busy ? "Finding answer…" : "Ask"}</button></div></form>
