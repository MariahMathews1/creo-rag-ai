...
test("historical examples keep local analysis while hiding the retired AI experiment", async () => {
  ...
  await user.click(screen.getByRole("menuitem", { name: "Historical Post Behavior Analysis" }));
  expect(screen.getByRole("heading", { level: 1, name: "Historical Post Behavior Analysis" })).toBeInTheDocument();
  expect(screen.getByText("SPINDL/RPM,{rpm}")).toBeInTheDocument();
  await user.click(screen.getByRole("button", { name: "Advanced" }));
  expect(screen.queryByRole("menuitem", { name: /AI Retrieval Experiment/ })).not.toBeInTheDocument();
  ...
  expect(screen.getByRole("heading", { name: "Legacy AI Metadata" })).toBeInTheDocument();
...
test("legacy consent metadata cannot authorize external CL/NCL processing", async () => {
  ...
  render(<MemoryRouter initialEntries={["/translations/3"]}><Routes><Route path="/translations/:exampleId" element={<TranslationDetailPage />} /></Routes></MemoryRouter>);
  const heading = await screen.findByText("Legacy AI Consent Metadata");
  const consent = heading.closest("section") as HTMLElement;
  expect(within(consent).getByText("Previously enabled")).toBeInTheDocument();
  expect(within(consent).getByText(/CL\/NCL transmission to any AI provider is prohibited/)).toBeInTheDocument();
  expect(within(consent).queryByRole("button", { name: /AI Processing/ })).not.toBeInTheDocument();
  expect(api.setTranslationAIConsent).not.toHaveBeenCalled();
});

test("direct legacy AI experiment route is a disabled historical notice", async () => {
  render(<MemoryRouter initialEntries={["/translations/ai-experiment"]}><Routes><Route path="/translations/*" element={<TranslationExamplesPage />} /></Routes></MemoryRouter>);
  expect(await screen.findByRole("heading", { name: "Deprecated / Previous R&D Experiment" })).toBeInTheDocument();
  expect(screen.getByText("AI_CL_NCL_TRANSMISSION_PROHIBITED")).toBeInTheDocument();
